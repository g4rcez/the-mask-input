...
export const adjustCaretPosition = ({
	...
	if (possiblyHasRejectedChar) {
		...
		const intersection = leftHalfChars.filter((char: any) => normalizedConformedValue.indexOf(char) !== -1);
		...
		const pipedChars = indexesOfPipedChars.map((index: any) => normalizedConformedValue[index]);
		const countTargetCharInPipedChars = pipedChars.filter((char: any) => char === targetChar).length;
		const countTargetCharInIntersection = intersection.filter((char: any) => char === targetChar).length;
